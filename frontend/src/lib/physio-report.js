...
function prescription(t = {}, unit) {
  ...
  return `${value(t.sets)} set(s); ${value(t.reps)} reps; ${load}`
}
function actual(s, t, unit) {
  ...
  return `${value(s.r)} reps; load ${value(s.w)} ${unit}`
}
...
export function preparePhysioReport(S, options, catalogue = []) {
  ...
  const sessions = (S.workouts || []).filter(inRange).slice().sort((a, b) => a.d.localeCompare(b.d)).map(w => {
    ...
    const rows = (w.entries || []).flatMap(e => {
      ...
      const output = completed.map((s, i) => [i === 0 ? name(e) : '', String(s.number), actual(s, e.target, unit), [Number.isFinite(s.rir) ? `RIR ${s.rir}` : '', Number.isFinite(s.rpe) ? `RPE ${s.rpe}` : ''].filter(Boolean).join('; ') || 'Not logged', i === 0 ? prescription(e.target, unit) : ''])
  ...
  return { from, to, sessions, results, routines, options: { notes, feedback, plan, tests }, assumedUnit: sessions.some(w => w.assumedUnit) }
}
