...
export default function TrainingPreferences() {
  ...
  const save = () => {
    ...
    const value = { ...draft, name: draft.name.trim(), equipment: draft.equipment.trim(), notes: draft.notes.trim() }
  ...
  return <Section title="Training preferences" footer="Equipment context is recorded with new sessions. Switching location does not change your weights, substitute exercises or remove mandatory work.">
    ...
    <div style={{ padding: 16, display: 'grid', gap: 12 }}>
      ...
      {!draft && <Button disabled={profiles.length >= 12} onClick={() => setDraft({ id: crypto.randomUUID(), name: '', equipment: '', notes: '', increment: '', unit: S.unit || 'kg' })}>Add home / gym / travel profile</Button>}
      {draft && <fieldset style={{ display: 'grid', gap: 12 }}><legend>Equipment profile</legend>
        ...
        <div style={{ display: 'flex', gap: 8 }}><Button onClick={() => setDraft(null)}>Cancel</Button><Button variant="primary" disabled={!draft.name.trim() || (draft.increment !== '' && (!Number.isFinite(Number(draft.increment)) || Number(draft.increment) < 0 || Number(draft.increment) > 100))} onClick={save}>Save profile</Button></div>
      </fieldset>}
    </div>
