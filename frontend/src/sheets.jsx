...
export const exConfigSheet = (ex, existing, onSave, onDelete, routine, addLabel) => ui().openSheet(close => <ExConfig ex={ex} existing={existing} onSave={onSave} onDelete={onDelete} routine={routine} addLabel={addLabel} close={close} />)
  ...
  return <><h3>{fmtDate(iso, true)}</h3>
    ...
    {!!done.length && <div className="list" style={{ marginBottom: 12 }}>{done.map(w => w.kind === 'activity'
      ...
      : <div className="card" key={w.id} style={{ margin: 0, padding: 14 }}>
        <div className="row between" style={{ gap: 8 }}><div><div className="tt">{w.name}</div><div className="ss">{[`${setsDone(w)} sets`, sessionLoadLabel(w, st.unit)].filter(Boolean).join(' · ')}</div></div><span className="tag" style={{ color: 'var(--green)' }}>{t('Completed')}</span></div>
...
export function WorkoutRow({ w, onClick }) {
  ...
  return <div className="item" onClick={onClick}>
    ...
    <div className="grow"><div className="tt">{w.name}</div>
      <div className="ss">{w.kind === 'activity' ? [fmtDate(w.d, true), ...durPart(w.end - w.start), t('Intensity') + ' ' + w.intensity + '/10'].join(' · ') : [fmtDate(w.d, true), ...durPart(w.end - w.start), t('{0} sets', setsDone(w)), sessionLoadLabel(w, st.unit)].filter(Boolean).join(' · ')}</div></div>
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    <div className="tiles" style={{ textAlign: 'left' }}>
      ...
      {w.vol > 0 && <div className="tile"><div className="l">{t('Volume')}</div><div className="v" style={{ fontSize: '1.1rem' }}>{sessionLoadLabel(w, st.unit)}</div></div>}
...
export function finishWorkout(incomplete = false) {
  ...
  if (A.editingWorkoutId) {
    if (!done) { toast(t('Add at least one set, or delete the workout instead.')); return }
    doFinishWorkout()
    return
  }
