...
- Generic denial-of-service reports with no amplification or rate-limit bypass. The API limits
  authentication attempts and signed-in writes; an internet-facing reverse proxy should still
  provide connection and request limits.
- **HSTS only.** The shipped nginx adds CSP, frame, MIME, referrer and permissions headers.
  HSTS remains the TLS-terminating reverse proxy's responsibility.
...
- **Passkey user verification is required by default.** Registration and authentication ask the
  authenticator for a biometric, device PIN or equivalent verification. An operator can explicitly
  set `REQUIRE_USER_VERIFICATION=false` only for a legacy authenticator that cannot comply.
- **Corrupt account storage fails closed.** An unreadable or structurally invalid `db.json` stops
  startup without replacing the original. Database writes use a same-directory temporary file,
  an fsync and atomic rename.
...
- **HTTPS is required and the app doesn't provide it.** The API container speaks plain HTTP and
  ...
  in the clear.
- **Rate limiting is deliberately modest and in-memory.** Login and registration handshakes and
  signed-in writes are limited per minute, but counters reset when the API restarts and they do
  not replace edge limits for an internet-facing instance. `TRUST_PROXY=true` is safe only when
  direct access is blocked and every ingress proxy overwrites untrusted forwarding headers.
- **A few endpoints answer without a session:** `/api/health` (only `{ ok: true }`), `/api/config`
  (instance capability flags), `/api/push/public-key`, and the
